...
function Dashboard() {
    ...
    const dailyAPR = (Math.pow(parseFloat(trimmedStakingAPY) / 100 + 1, 1 / 1095) - 1) * 300;
    const percentLocked = (app.stakingTVL / app.marketCap) * 100;
                    ...
                    <Grid container spacing={4}>
                        <Grid item lg={12} md={6} sm={6} xs={12}>
                            <div className="dashboard-card">
                                <p className="card-title">BARS Price</p>
                        ...
                        <Grid item lg={6} md={6} sm={6} xs={12}>
                            <div className="dashboard-card">
                                <p className="card-title">Percent Locked</p>
                                <p className="card-value">{isAppLoading ? <Skeleton width="100px" /> : `${new Intl.NumberFormat("en-US").format(Number(percentLocked))}%`}</p>
                            </div>
                        </Grid>
                        ...
                        <Grid item lg={6} md={6} sm={6} xs={12}>
                            <div className="dashboard-card">
                                ...
                                <p className="card-value">{isAppLoading ? <Skeleton width="250px" /> : `${trim(Number(app.currentIndex), 2)} BARS`}</p>
                            </div>
                        </Grid>
                        ...
                        <Grid item lg={12} md={6} sm={6} xs={12}>
                            <div className="dashboard-card">
                                ...
                                <p className="card-value">{isAppLoading ? <Skeleton width="250px" /> : `${new Intl.NumberFormat("en-US").format(Number(dailyAPR))}%`}</p>
                            </div>
                        </Grid>

                        <Grid item lg={6} md={6} sm={6} xs={12}>
                            <div className="dashboard-card">
                                <p className="card-title">Backing per $BARS</p>
                                <p className="card-value">
                                    {isAppLoading ? (
                                        <Skeleton width="250px" />
                                    ) : (
